...
export function AssignmentsClient({ focus, initialMode, userId }: { focus: MonthFocus; initialMode: 'primary' | 'secondary' | 'combined'; userId: string }) {
  ...
  const loadDaily = useCallback(async () => {
    ...
    setDaily(null)
    try {
      const res = await fetch('/api/assignments/instructor', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ primary: focus.primary, secondary: focus.secondary, month: focus.month, dailyExercise }) })
      if (!res.ok) return
    ...
    <div className="max-w-[1100px] mx-auto px-8 py-11 pb-16">
      ...
      <div className="card mb-5">
        ...
        <div className="flex items-start justify-between gap-6 mb-5">
          ...
          <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(artist.youtube_search)}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm shrink-0">Watch on YouTube →</a>
        </div>
      ...
      <div className="flex gap-1 mb-5 border-b border-line">
        {(['daily', 'weekly', 'capstone'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2.5 text-sm font-medium capitalize transition-colors border-b-2 -mb-px ${activeTab === tab ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-ink'}`}>
          ...
          <div className="card mb-5">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                ...
                <span className="inline-flex items-center gap-2 bg-accent/10 text-accent rounded-full px-3.5 py-1 text-xs font-semibold tracking-wide mt-2">{modeLabel}</span>
              </div>
            ...
            {loading ? <Skeleton /> : daily ? (
              <div className="space-y-4 mb-6">
                ...
                <div className="teacher-block border-l-[3px] border-gold/60">
                  ...
                  <ul className="space-y-1">{daily.mistakes.map((m, i) => <li key={i} className="text-sm">— {m}</li>)}</ul>
                </div>
                <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {!completed ? <button onClick={markComplete} className="btn btn-primary btn-sm">Mark complete</button> : <span className="btn btn-sm bg-gold/10 text-[#9a7510]">Done for today ✓</span>}
            </div>
            {completed && daily && <p className="text-sm text-muted mt-4 italic">{daily.encouragement}</p>}
          </div>
          <div className="card mb-5">
            <p className="eyebrow mb-2">After Your Study</p>
            {daily && <p className="font-serif text-xl font-normal italic text-secondary leading-snug mb-4">"{daily.reflection}"</p>}
            <p className="font-serif text-lg font-normal italic text-muted leading-snug mb-4">"{THOUGHT_PROMPTS[promptIdx]}"</p>
            <textarea value={reflection} onChange={e => setReflection(e.target.value)} placeholder="10 minutes of honest writing. No editing." className="w-full border border-line rounded-xl bg-cream px-4 py-3 text-sm text-ink outline-none resize-y min-h-[100px] focus:border-accent transition-colors mb-4" />
            <div className="flex gap-2">
              <button onClick={saveReflection} disabled={saving || !reflection.trim()} className="btn btn-primary btn-sm">{reflectionSaved ? 'Saved ✓' : saving ? 'Saving…' : 'Save reflection'}</button>
              <button onClick={() => setPromptIdx((promptIdx + 1) % THOUGHT_PROMPTS.length)} className="btn btn-ghost btn-sm">New prompt</button>
            </div>
          </div>
        </>
      )}
      {activeTab === 'weekly' && (
        <div className="card mb-5">
          ...
          <div className="progress-bar mb-1.5"><div className="progress-fill" style={{ width: '62%' }} /></div>
      ...
      {activeTab === 'capstone' && (
        <div className="card mb-5">
          ...
          <p className="font-serif text-2xl font-normal leading-snug mb-4">{focus.primary} + {focus.secondary}</p>
          <p className="text-sm text-muted leading-relaxed">This month's capstone combines both fundamentals into one finished piece. Your work should demonstrate clear {focus.primary.toLowerCase()} and intentional {focus.secondary.toLowerCase()}. Upload your final piece plus at least two process studies showing your development.</p>
        </div>
      )}
    </div>
  )
